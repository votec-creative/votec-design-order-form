  ...
  return `
    ...
    ${prefix.startsWith('card-') ? `
      <div class="instruction-apply-all-footer">
        ...
        <button type="button" class="instruction-apply-all" onclick="applyInstructionToAllImages()" ${state.imgCards.length > 1 && !card.sameAsCardKey && hasRequiredInstruction(card) ? '' : 'disabled'}>
...
function refreshInstructionCompletionIndicators() {
  ...
  if (applyAllButton && activeCard) {
    ...
    applyAllButton.disabled = !canApplyToAll;
...
function applyInstructionToAllImages() {
  ...
  if (!window.confirm(`「${sourceLabel}」の指示を、ほかのすべての制作画像に適用しますか？`)) return;

  state.imgCards.forEach(card => {
    if (card !== sourceCard) card.sameAsCardKey = sourceKey;
  });
  saveDraft();
  renderInstructionGroups();
}
